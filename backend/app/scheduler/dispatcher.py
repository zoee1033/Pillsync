...
def dispatch_reminder(db, reminder):
    ...
    print(f"Active device token(s) for user {user_id}: {len(active_tokens)} found -> IDs {token_ids}", flush=True)

    if not active_tokens:
        print(f"No active device token found for user {user_id}", flush=True)
        return
    ...
    for device in active_tokens:
        ...
        print(f"Attempting push notification for user {user_id} using token ID {device.id} ({masked_tok})", flush=True)
        ...
        try:
            ...
            print(f"Reminder sent successfully to user {user_id} using device token ID {device.id}", flush=True)
            break
            ...
            if is_unregistered:
                print(f"Token ID {device.id} is invalid/unregistered ({e}). Automatically deactivating token...", flush=True)
                device.is_active = False
                try:
                    db.commit()
                except Exception as commit_err:
                    db.rollback()
                    print(f"Failed to deactivate token ID {device.id}: {commit_err}", flush=True)
                print(f"Continuing to next available device token for user {user_id}...", flush=True)
            else:
                print(f"Firebase error sending push to token ID {device.id}: {e}", flush=True)